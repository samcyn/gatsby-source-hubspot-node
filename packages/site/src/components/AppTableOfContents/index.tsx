      ...
      <nav className="md:px-8">
        ...
        <ul className="flex flex-col gap-3">
          {contents.map((content) => (
            <li key={content.url}>
              <a className="block text-sm text-gray-60 dark:text-gray-50 hover:text-primary hover:underline" href="#">
                {content.title}
              </a>
              {content.items ? (
                <ul className="flex flex-col gap-3 mt-3 ml-4">
                  {content.items.map((record) => (
                    <li key={record.url}>
                      <a
                        className="block text-sm text-gray-60 dark:text-gray-50 hover:text-primary hover:underline"
                        href="#"
                      >
                        {record.title}
                      </a>
